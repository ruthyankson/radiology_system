...
def assigned_staff_choices():
    return [(item.first_name, item.first_name + " " + item.last_name) for item in User.objects.all()]
...
def department_choices():
    return [(item.department_name, item.department_name) for item in Department.objects.all()]
